...
def gen_dict(state):
    imgsA = os.listdir(osp.join(root_path, state, nameA))
    imgsB = os.listdir(osp.join(root_path, state, nameB))
    labs = os.listdir(osp.join(root_path, state, label))

    imgsA = sorted([osp.join(root_path, state, nameA, i) for i in imgsA])
    imgsB = sorted([osp.join(root_path, state, nameB, i) for i in imgsB])
    labs = sorted([osp.join(root_path, state, label, i) for i in labs])
...
def cut_data(cut_size, over_lap, save_dir, state):
    path_list = gen_dict(state)

    os.makedirs(osp.join(save_dir, state, nameA))
    os.makedirs(osp.join(save_dir, state, nameB))
    os.makedirs(osp.join(save_dir, state, label))
    ...
    for i in tqdm.tqdm(range(len(path_list))):
        ...
        while left <= w_new:
            ...
            while down <= h_new:
                ...
                nj += 1
                yimage.io.write_image(osp.join(save_dir, state, nameA, '{}_{}_{}.{}'.format(img_name, ni, nj, lab_suffix)), img_sA)
                yimage.io.write_image(osp.join(save_dir, state, nameB, '{}_{}_{}.{}'.format(img_name, ni, nj, lab_suffix)), img_sB)
                yimage.io.write_image(osp.join(save_dir, state, label, '{}_{}_{}.{}'.format(img_name, ni, nj, lab_suffix)), lab_s)
...
if __name__ == '__main__':
    ...
    lab_suffix = 'png'
    for state in split:
        cut_data(cut_size, over_lap, save_dir, state)
