...
if [ "$data_name" == "WHUCD" ]; then
    ...
    bash tools/dist_test.sh "$config_file" "$model_checkpoint" $num_gpu --out "$work_dirs/test_result" --cfg-options test_dataloader.dataset.data_root="$CDPATH/SYSU-CD"
    python tools/general/metric.py --pppred "$work_dirs/test_result" --gggt "$label_dir"
    ...
    label_dir="/home/ps/HDD/zhaoyq_data/CDdata/CD_Data_GZ/cut_data/labels_change"
    bash tools/dist_test.sh "$config_file" "$model_checkpoint" $num_gpu --out "$work_dirs/test_result"
    python tools/general/metric.py --pppred "$work_dirs/test_result" --gggt "$label_dir"
