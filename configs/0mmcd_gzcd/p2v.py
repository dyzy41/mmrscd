...
import os
data_root = os.path.join(os.environ.get("CDPATH"), 'CD_Data_GZ/cut_data')
crop_size = (256, 256)
...
model = dict(
    ...
    decode_head=dict(
        ...
        threshold=0.5,
        loss_decode=dict(
            type='CrossEntropyLoss', use_sigmoid=True, loss_weight=1.0, loss_name='loss_bce')),
    auxiliary_head=dict(
        type='NullDecodeHead',
        in_index=1,
        in_channels=256,
        ...
        threshold=0.5,
        loss_decode=dict(
            type='CrossEntropyLoss', use_sigmoid=True, loss_weight=0.5, loss_name='loss_bce')),
