...
import os
data_root = os.path.join(os.environ.get("CDPATH"), 'CLCD')  #修改数据集路径
crop_size = (256, 256)
...
model = dict(
    ...
    train_cfg=dict(),
    test_cfg=dict(mode='slide', crop_size=(256, 256), stride=(170, 170)))    #  CLCD要改这里
...
train_pipeline = [
    ...
    # dict(type='CLAHE'),
    dict(type='RandomCrop', crop_size=crop_size),    #  CLCD要改这里
...
train_dataloader = dict(batch_size=16,   #修改batchsize
                        num_workers=16,
...
train_cfg = dict(type='IterBasedTrainLoop', max_iters=20000, val_interval=1000)  #修改训练次数的
