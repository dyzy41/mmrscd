...
@TRANSFORMS.register_module()
class RandomCrop(BaseTransform): #CD
...
@TRANSFORMS.register_module()
class RandomRotate(BaseTransform): #CD
...
@TRANSFORMS.register_module()
class AdjustGamma(BaseTransform): #CD
    ...
    def transform(self, results: dict) -> dict:
        ...
        Returns:
            dict: Processed results.
        """
        band_width = results['img'].shape[-1]
        if band_width==3:
            results['img'] = mmcv.lut_transform(
                np.array(results['img'], dtype=np.uint8), self.table)
        else:
            imgA = mmcv.lut_transform(
                np.array(results['img'][:, :, :3], dtype=np.uint8), self.table)
            imgB = mmcv.lut_transform(
                np.array(results['img'][:, :, 3:], dtype=np.uint8), self.table)
            results['img'] = np.concatenate([imgA, imgB], axis=2)
        return results
...
@TRANSFORMS.register_module()
class PhotoMetricDistortion(BaseTransform): #CD
...
@TRANSFORMS.register_module()
class RandomRotFlip(BaseTransform): #CD
...
@TRANSFORMS.register_module()
class RandomFlip(MMCV_RandomFlip): #CD
